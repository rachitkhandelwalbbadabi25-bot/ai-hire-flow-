...
import { 
  Mail, Inbox, Send, Plus, Users, UserCheck, ExternalLink, Zap, X, 
  Loader2, Copy, CheckCircle2, Trash2, Sparkles, FileText, RefreshCw 
} from 'lucide-react';
...
export default function OutreachHub() {
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      try {
        ...
        const snap = await getDocs(collection(db, 'users', user.uid, 'outreach_contacts'));
  ...
  const handleAddContact = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !newName.trim() || !newCompany.trim() || !newEmail.trim()) return;
    ...
    try {
      const docRef = await addDoc(collection(db, 'users', user.uid, 'outreach_contacts'), newContact);
  ...
  const handleToggleStatus = async (contactId: string, currentStatus: string) => {
    ...
    try {
      await updateDoc(doc(db, 'users', user.uid, 'outreach_contacts', contactId), {
        status: nextStatus
      });
  ...
  const handleDeleteContact = async (contactId: string) => {
    if (!user) return;
    try {
      await deleteDoc(doc(db, 'users', user.uid, 'outreach_contacts', contactId));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
      <div className="mb-12">
        <h1 className="text-3xl font-bold text-ink tracking-tight mb-2 uppercase">Neural Outreach Hub</h1>
        <p className="text-ink-dim font-medium text-sm">Draft professional cold Gmail pitches and coordinate warm corporate connections dynamically.</p>
      </div>
            ...
            >
               <button 
                ...
                <form onSubmit={handleAddContact} className="space-y-4">
                  <div>
                    <label className="text-[10px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Connection Name</label>
                    <input 
                      type="text" 
                      required
                      value={newName}
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Corporate Hub / Company</label>
                    <input 
                      type="text" 
                      required
                      value={newCompany}
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Gmail / Email Identifier</label>
                    <input 
                      type="email" 
                      required
                      value={newEmail}
               ...
               <button 
                ...
                <div className="mb-6">
                  ...
                  <p className="text-[10px] font-bold text-accent uppercase tracking-widest mt-1">
                    Generating customized referral email for {selectedContact?.name} @ {selectedContact?.company}
                  </p>
                </div>

                {generatingPitch ? (
                  <div className="bg-background border border-border p-12 rounded-3xl min-h-[300px] flex flex-col items-center justify-center gap-4">
                     <Loader2 className="w-8 h-8 text-accent animate-spin" />
                     <span className="text-[10px] font-bold text-ink-dim uppercase tracking-widest animate-pulse">Running Narrative Synthesis...</span>
                  </div>
                ) : (
                  <div className="space-y-4">
                     {/* Subject field */}
                     <div className="bg-background border border-border p-4 rounded-2xl">
                       <label className="text-[9px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Subject line</label>
                       <div className="flex gap-2 items-center">
                         ...
                         <button 
                           ...
                           title="Copy Subject"
                         >
                           {copiedSubject ? <CheckCircle2 className="w-3.5 h-3.5 text-success" /> : <Copy className="w-3.5 h-3.5" />}
                           <span className="text-[9px] font-bold uppercase tracking-wider">{copiedSubject ? 'Copied' : 'Copy'}</span>
                         </button>
                       </div>
                     </div>
                     ...
                     {/* Body field */}
                     <div className="bg-background border border-border p-4 rounded-2xl">
                       <label className="text-[9px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Cold outreach body</label>
                       <textarea 
                         value={generatedBody}
                       ...
                       <div className="flex justify-end gap-2 mt-3">
                         <button 
                           ...
                           className="p-2.5 bg-surface border border-border text-ink-dim hover:text-accent rounded-xl text-xs transition-colors flex items-center gap-1.5"
                         >
                           {copiedBody ? <CheckCircle2 className="w-3.5 h-3.5 text-success" /> : <Copy className="w-3.5 h-3.5" />}
                           <span className="text-[9px] font-bold uppercase tracking-widest">{copiedBody ? 'Body Copied' : 'Copy Body'}</span>
                         </button>
                       </div>
                     </div>
                     ...
                     <div className="mt-6 flex flex-col sm:flex-row gap-3">
                        ...
                        >
                          {copiedAll ? <CheckCircle2 className="w-4 h-4 text-success" /> : <Copy className="w-4 h-4" />}
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-8">
          ...
          <div className="glass-panel shadow-sm">
             <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                   <h2 className="text-lg font-bold text-ink uppercase tracking-tight">Gmail Neural Gateway</h2>
                   <p className="text-[9px] font-bold text-ink-dim uppercase tracking-widest">Connect Pipeline to Personal Mail</p>
                </div>
             </div>

             <div className="bg-background border border-border p-5 rounded-2xl mb-6">
                <label className="text-[9px] font-bold text-ink-dim uppercase tracking-widest block mb-2">Connected Gmail Account</label>
             ...
             <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between p-3 bg-surface-light/50 rounded-xl border border-border">
                   <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-[11px] font-bold text-ink uppercase tracking-tight">Pitch Draft Auto-Synthesis</span>
                   </div>
             ...
             >
                {syncing ? <Loader2 className="w-4 h-4 animate-spin" /> : "Verify Connection"} 
                {!syncing && <Send className="w-3.5 h-3.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />}
             </button>
          </div>
          ...
          <div className="glass-panel shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold uppercase text-ink tracking-tight">Outreach Background</h3>
                  <p className="text-[9px] font-mono text-ink-dim uppercase tracking-widest">Controls AI Personalization Pitch</p>
                </div>
              </div>
            ...
            <p className="text-xs text-ink-dim mb-4 leading-relaxed font-medium">
              We personalize cold referral requests based on this bio. It has been preloaded with content from your latest analyzed resume. Feel free to refine it below:
            </p>
          ...
          <div className="bg-[#0c0c0c] border border-border rounded-[2rem] p-6 text-white shadow-lg">
             <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xs font-bold uppercase tracking-widest text-white">Cold Pitch Intelligence</h3>
             </div>
             <p className="text-xs text-white/50 leading-relaxed font-sans mb-4 italic">
                "We leverage structural narrative synthesis. By comparing your precise elevator pitch against the target contact's corporate profile, we generate highly strategic referrals that bypass initial filter barriers."
             </p>
             <div className="p-3 bg-white/5 rounded-xl border border-white/10 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-success animate-pulse" />
                <span className="text-[9px] font-bold text-white/70 uppercase font-mono">Neural Composer Ready</span>
             </div>
          </div>
        </div>
          ...
          <div className="glass-panel shadow-sm min-h-[500px]">
             <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      ...
                      <p className="text-[10px] font-bold text-ink-dim uppercase tracking-[0.2em] mt-0.5">Target Contact Hub</p>
                   </div>
                </div>
                <button 
             ...
             {loadingContacts ? (
               <div className="flex flex-col items-center justify-center p-20 gap-3">
                 <Loader2 className="w-8 h-8 text-accent animate-spin" />
                 <span className="text-[10px] font-bold text-ink-dim uppercase tracking-widest">Loading Referral Matrix...</span>
               </div>
             ) : contacts.length === 0 ? (
               <div className="flex flex-col items-center justify-center p-20 text-center">
                 <Inbox className="w-12 h-12 text-border mb-4" />
                 <p className="text-sm font-bold text-ink uppercase tracking-widest mb-1">Matrix Empty</p>
                 <p className="text-xs text-ink-dim max-w-xs leading-relaxed mb-6">Create connections to start generating deep email referrals.</p>
                 <button 
                   onClick={() => setShowAddModal(true)}
                   className="px-5 py-2.5 bg-accent text-white rounded-xl text-[10px] font-bold uppercase tracking-widest"
                 >
                   Add Connection
                 </button>
               </div>
                  ...
                  <table className="w-full">
                     <thead>
                        <tr className="border-b border-border">
                           <th className="pb-4 text-left text-[10px] font-bold text-ink-dim uppercase tracking-widest">Connection</th>
                           ...
                           <th className="pb-4 text-right text-[10px] font-bold text-ink-dim uppercase tracking-widest">Actions</th>
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-border">
                        {contacts.map((ref) => (
                           <tr key={ref.id} className="group hover:bg-accent/[0.01] transition-colors">
                              <td className="py-5">
                                 <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                       ...
                                       <span className="text-[10px] font-mono text-ink-dim mt-0.5">{ref.email}</span>
                                    </div>
                                 </div>
                              </td>
                              <td className="py-5">
                                 <span className="text-xs font-semibold text-ink uppercase tracking-tight">{ref.company}</span>
                              </td>
                              <td className="py-5">
                                 <button 
                                   ...
                                   className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider cursor-pointer border hover:border-accent transition-colors ${
                                    ref.status === 'Referred' ? 'bg-success/10 text-success border-success/20' : 
                                    ref.status === 'Requested' ? 'bg-warning/10 text-warning border-warning/20' : 
             ...
             <div className="mt-12 p-6 bg-surface-light border border-border rounded-2xl border-dashed">
                <div className="flex flex-col items-center text-center">
                   ...
                   <h4 className="text-xs font-bold text-ink uppercase tracking-widest mb-1.5">Elevate Response Probability</h4>
                   <p className="text-xs text-ink-dim max-w-sm leading-relaxed mb-4">
                      Add actual professionals from target tech companies. A customized email asking for advice or feedback can increase interview rates by up to 5x.
                   </p>
                </div>
             </div>
          </div>
